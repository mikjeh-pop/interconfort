    ...
    <>
        ...
        <Container>
          <div className="max-w-4xl py-16 sm:py-24">
            ...
            <p className="mt-8 max-w-2xl text-lg leading-relaxed text-ink-2">
              Installations en service chez des traiteurs, des pharmaciens, des
              chocolatiers, des boulangers et des producteurs. Le site actuel
              d’Inter-Confort recense {totalRealisationsSiteActuel} réalisations
              photographiées : elles seront reprises ici, classées par solution
              et par métier.
            </p>
          </div>
        </Container>
      ...
      {categories.map((cat) => {
        ...
        return (
          <section
            key={cat}
            className="border-b border-ink/10 py-14 sm:py-20 odd:bg-paper-2"
          >
            <Container>
                      ...
                      {r.image ? (
                        <>
                          <div className="relative aspect-[3/4] w-full bg-paper-2">
                            <Image
                              src={r.image.src}
                              unoptimized={r.image.src.startsWith("http")}
                              alt={r.image.alt}
                              fill
                        ...
                        <>
                          <PhotoManquante
                            description={r.aFournir ?? r.titre}
                            ratio="3 / 4"
                          />
                          <p className="mt-3 text-sm text-ink">{r.titre}</p>
                        </>
