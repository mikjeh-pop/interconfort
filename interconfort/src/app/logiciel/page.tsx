            ...
            {logiciel.groupes.map((g, gi) => (
              <Reveal key={g.titre} delay={gi * 60}>
                <div className="grid gap-8 lg:grid-cols-[0.6fr_1.4fr] lg:gap-16">
                  <TitreSection eyebrow={String(gi + 1).padStart(2, "0")}>
                    {g.titre}
                  </TitreSection>
                  <dl className="border-t border-ink/15">
                    {g.fonctions.map((f) => (
                      <div
                        key={f.nom}
                        className="grid gap-1 border-b border-ink/10 py-4 sm:grid-cols-[1fr_1.5fr] sm:gap-8"
                      >
          ...
          <div className="grid gap-12 py-20 sm:py-24 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
            <div>
              ...
              <p className="mt-8 max-w-lg leading-relaxed text-frost">
                L’interface web regroupe les machines d’un même exploitant.
                Chaque appareil expose son état, son stock, ses ventes et sa
                température. Les alertes remontent par notification, et les
                historiques peuvent être exportés.
              </p>
              <p className="mt-6 max-w-lg text-sm leading-relaxed text-frost/70">
                La représentation ci-contre est un schéma d’organisation de
                l’interface. Elle ne comporte volontairement aucune donnée : les
                captures réelles seront fournies par Inter-Confort.
              </p>
            </div>

            <div className="border border-white/20 bg-cold-2/60 p-5 sm:p-7">
              <p className="eyebrow !text-frost/70">
              ...
              <ul className="mt-6 space-y-px">
                {[
                  {
                    zone: "Parc",
                    contenu: "Liste des machines, état de connexion",
                  },
                  {
                    zone: "Machine",
                    contenu: "Stock par référence, dernières ventes",
                  },
                  {
                    zone: "Froid",
                    contenu: "Relevés de température, historique, alertes",
                  },
                  {
                    zone: "Catalogue",
                    contenu: "Références, prix, images, descriptions",
                  },
                  {
                    zone: "Emplacements",
                    contenu: "Affectation des produits aux sélections",
                  },
                  {
                    zone: "Retraits",
                    contenu: "Codes de réservation et validations",
                  },
                ].map((r) => (
                  <li
                    key={r.zone}
