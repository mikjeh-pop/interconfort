...
import { heroReferences } from "@/content/references";
...
const chiffres = [
  { valeur: 1986, label: "Année de création", brut: true },
  { valeur: 4, prefixe: "0–", suffixe: " °C", label: "Froid maintenu, extérieur compris" },
...
const methode = [
  ...
  },
  {
    titre: "Suivi",
    texte:
      "Service après-vente, pièces et accompagnement dans la durée. Une machine autonome n’est utile que si elle reste en service.",
  },
];
...
export default function Accueil() {
  return (
    <>
      ...
      <section className="relative flex min-h-[84svh] items-end overflow-hidden bg-ink text-white">
        <HeroCarousel items={heroReferences} />
        ...
        <Container className="relative z-10">
          <div className="max-w-4xl py-16 sm:py-24">
            ...
            <Reveal delay={120}>
              <h1 className="display mt-7 text-[clamp(2.5rem,7vw,5.6rem)] text-white">
                Vendre en continu,
                <br />
                sans rompre le froid.
              </h1>
            </Reveal>
            <Reveal delay={240}>
              <p className="mt-8 max-w-xl text-lg leading-relaxed text-white/80 sm:text-xl">
                Inter-Confort conçoit et installe des points de vente automatisés capables de tenir
                une température exacte, de manipuler des produits fragiles et de fonctionner sans
                personnel — en boutique comme en plein air.
              </p>
            </Reveal>
            <Reveal delay={360}>
              <div className="mt-11 flex flex-wrap gap-4">
                ...
                <Link
                  href="/solutions"
                  className="inline-flex items-center gap-3 px-2 py-4 text-sm text-white/85 underline-offset-8 transition-colors hover:text-white hover:underline"
                >
                  Voir les solutions
          ...
          <dl className="grid grid-cols-2 divide-ink/10 py-10 sm:grid-cols-4 sm:divide-x">
            {chiffres.map((c) => (
              <div key={c.label} className="px-0 py-4 sm:px-8 sm:py-0 sm:first:pl-0">
                ...
                <dd>
                  <span className="data block text-2xl text-ink sm:text-4xl">
                    {c.brut ? (
                      c.valeur
                    ) : (
            ...
            <Reveal>
              <div className="space-y-6 text-lg leading-relaxed text-ink-2">
                <p>
                  ...
                  Tout change avec une barquette de sushis, un ballotin de pralines ou un bocal de
                  miel : il faut tenir une température exacte, saisir l’article sans le heurter et
      ...
      <section className="border-b border-ink/10 bg-paper-2 py-20 sm:py-28">
        <Container>
          <div className="flex flex-wrap items-end justify-between gap-6">
            <TitreSection eyebrow="Installations en service">
              Elles tournent, dehors, toute l’année.
            </TitreSection>
      ...
      <section className="border-b border-ink/10 py-20 sm:py-28">
        <Container>
          <div className="flex flex-wrap items-end justify-between gap-6">
            <TitreSection eyebrow="Par métier">Chaque métier a sa contrainte.</TitreSection>
            <Link href="/secteurs" className="text-sm text-cold underline-offset-4 hover:underline">
              Toutes les pages métier →
            </Link>
          </div>
          <ul className="mt-12 grid gap-px bg-ink/10 sm:grid-cols-2 lg:grid-cols-4">
            {secteurs.map((sec, i) => (
              <li key={sec.slug} className="bg-paper">
                <Reveal delay={i * 35}>
                  <Link
                  ...
                  >
                    ...
                    <span className="display mt-3 text-lg text-ink transition-colors group-hover:text-cold">
                      {sec.titreCourt}
                    </span>
                    ...
                    <span
                      aria-hidden="true"
                      className="data mt-4 text-xs text-cold opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                    >
                      →
                    </span>
                  </Link>
            ...
            {solutions.map((s, i) => (
              <Reveal key={s.slug} delay={i * 90}>
                <Link href={s.href} className="group block">
                  ...
                  <h3 className="display mt-2 text-2xl text-ink transition-colors group-hover:text-cold">
                    {s.nom}
                  </h3>
      ...
      <section className="bg-cold text-white">
        <RuleTick clair />
        <Container>
          <div className="grid gap-14 py-20 sm:py-28 lg:grid-cols-2 lg:gap-20">
            <div>
            ...
            <Reveal>
              <div className="grid gap-4">
                <div className="relative aspect-[4/5] w-full overflow-hidden bg-cold-2">
                  <Image
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="relative aspect-square w-full overflow-hidden bg-cold-2">
                    <Image
        ...
        <Container>
          ...
          <ul className="mt-14 grid grid-cols-2 gap-4 md:grid-cols-5">
            {Object.values(machinesMagex).map((m, i) => (
              <li key={m.src}>
                <Reveal delay={i * 60}>
                  <div className="group relative aspect-[3/4] w-full bg-paper">
                    <Image
                      src={m.src}
                      unoptimized
                      alt={m.alt}
        ...
        <Container>
          ...
          <ol className="mt-12 border-t border-ink/15">
            {methode.map((m, i) => (
              <li
                key={m.titre}
                className="group grid gap-3 border-b border-ink/10 py-6 transition-colors hover:bg-paper sm:grid-cols-[3rem_1fr_1.4fr] sm:gap-8"
              >
                <span className="data text-sm text-steel transition-colors group-hover:text-hatch">
                  {String(i + 1).padStart(2, "0")}
                </span>
