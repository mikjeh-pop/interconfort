...
const capacites = [
  ...
  { format: "Petits pots et desserts · 150 × 100 mm", capacite: "Jusqu’à 200 à 240" },
  { format: "Très petits formats", capacite: "Capacité supérieure selon configuration" },
];

const plateaux = [
  { type: "Doubles tapis", usage: "Produits larges : grandes barquettes, plateaux traiteur, gratins." },
  { type: "Tapis simples couplables", usage: "Formats standards : salades, bowls, pâtisseries individuelles." },
  { type: "Poussoirs modulables en largeur", usage: "Emballages rigides : bouteilles, boîtes hermétiques, conserves." },
  { type: "Spirales d’épaisseurs variables", usage: "Fromages affinés, charcuteries, viandes conditionnées." },
];
    ...
    <PageSolution
      ...
      sections={
        <>
              ...
              <div className="grid gap-12 lg:grid-cols-[1fr_1fr] lg:gap-20">
                <div>
                  ...
                  <div className="mt-10 space-y-8 border-t border-ink/15 pt-8">
                    <div>
                      ...
                      <h3 className="display mt-2 text-lg text-ink">Plage de réglage</h3>
                      <p className="mt-2 max-w-lg text-sm leading-relaxed text-ink-2">
                        La consigne se règle selon la nature des produits. Cela ne signifie pas que
                        des produits frais se conservent à 20 °C.
                      </p>
                    </div>
                    <div>
                      <p className="data text-xl text-ink">0 à 4 °C</p>
                      <h3 className="display mt-2 text-lg text-ink">Performance standard</h3>
                      <p className="mt-2 max-w-lg text-sm leading-relaxed text-ink-2">
                        Le groupe frigorifique à double évaporateur homogénéise et stabilise le
                        froid, y compris en extérieur et en plein soleil.
                      </p>
                    </div>
                    <div>
                      <p className="data text-xl text-ink">Au-delà de 40 °C</p>
                      <h3 className="display mt-2 text-lg text-ink">Expositions extrêmes</h3>
                      <p className="mt-2 max-w-lg text-sm leading-relaxed text-ink-2">
                        Une configuration renforcée — second système ventilé, sondes digitales
                        calibrées et régulation pilotée par les sondes — sécurise le maintien entre
                        0 et 4 °C lorsque la température ambiante dépasse 40 °C.
                      </p>
                    </div>
                  </div>
                </div>
                <Reveal>
                  ...
                  <p className="mt-3 text-xs text-steel">
                    Installation extérieure — le toit de protection est prévu à cet effet.
                  </p>
                </Reveal>
              </div>
            </Container>
          ...
          <section className="border-b border-ink/10 bg-paper-2 py-20 sm:py-24">
            <Container>
              <div className="grid gap-12 lg:grid-cols-[1fr_1fr] lg:gap-20">
                ...
                <div>
                  <TitreSection eyebrow="Stockage">Des plateaux adaptés à chaque produit.</TitreSection>
                  <p className="mt-8 max-w-xl leading-relaxed text-ink-2">
                    L’unité de gestion détecte automatiquement le type de plateau installé et met le
                    stock à jour. Le remplissage se fait plateau par plateau, sur des emplacements
                    numérotés.
                  </p>
                  <dl className="mt-10 border-t border-ink/15">
                    {plateaux.map((p) => (
                      <div key={p.type} className="grid gap-1 border-b border-ink/10 py-4 sm:grid-cols-[1fr_1.4fr] sm:gap-8">
                        <dt className="data text-sm text-ink">{p.type}</dt>
                        <dd className="text-sm leading-relaxed text-ink-2">{p.usage}</dd>
                      </div>
                    ))}
                  </dl>
                </div>
          ...
          <section className="border-b border-ink/10 py-20 sm:py-24">
            <Container>
              ...
              <p className="mt-8 max-w-2xl leading-relaxed text-ink-2">
                Les volumes ci-dessous sont indicatifs : la largeur, la profondeur et la hauteur des
                conditionnements déterminent l’agencement retenu. Une étude de stockage est réalisée
                avant commande.
              </p>
              <dl className="mt-10 border-t border-ink/15">
                {capacites.map((c) => (
                  <div key={c.format} className="grid gap-1 border-b border-ink/10 py-4 sm:grid-cols-[1.4fr_1fr] sm:gap-8">
          ...
          <section className="border-b border-ink/10 bg-paper-2 py-20 sm:py-24">
            <Container>
              <TitreSection eyebrow="Exploitation">Paiement, retrait et pilotage.</TitreSection>
